...
| Burn | hint(wrong-lancet-Burn) | acts | hint(wrong-leech-Burn) | hint(wrong-thread-Burn) | hint(wrong-salve-Burn,burn-eschar) | hint(wrong-tincture-Burn,inject-waste) penalty(BAD) | hint(wrong-brand-Burn,brand-flesh) penalty(BAD) | hint(wrong-lens-Burn) |
