...
- [x] NAR-0177 · Alpha · P0 · L · Ch3–5 second draft — incorporates playtest notes and ending matrix; all scene ids wired in campaign data _(the ending matrix is in (NAR-0158) and every Ch3–5 scene the chapter modules define (39) is wired into the campaign, no orphans (tests/unit/content/continuity.test.ts); the second draft is the NAR-0178/0179 passes. The playtest-notes revision needs players and is parked with the playtests as NAR-0192)_
- [x] NAR-0178 · Beta · P0 · M · Ch3–5 polish pass — pacing budget enforced, jokes audited against humour rule, Kreuzer voice consistent _(budget: ≤ 14 lines in the longest flag variant, the crypt and endings ≤ 24 (tests/helpers/storyVariants.ts); s4-8 trimmed 18 → 14. All 611 Ch3–5 lines read: 41 joke lines audited in the style guide, one fail rewritten (Emmerich’s epilogue); Stroh’s thaw written into the voice table. Kreuzer never exclaims and keeps spoken lines ≤ 130 chars, tested)_
- [x] NAR-0179 · Beta · P1 · S · Full-game continuity pass — timeline, flags and callbacks verified against the bible; issues tracked to zero _(fixed: the road home — s4-end moves to 24 Saatmond, six days out and four on the road, so s5-1’s “two days before” holds; Mauer’s head-count now follows the ford through s5-end (`mauerCount`), and number-free where it can’t (a barks pair, op4-7’s ledger, his epilogue); `onlyIf` composes nested conditions instead of dropping the inner one (s5-3 would have shown both counts); flags.md gains `unsungHeard` and the in-play story flags. Liesl’s age, Stroh’s molar, Haller’s hands and the Office’s hour count checked. tests/unit/content/continuity.test.ts)_
