...
- [x] GAM-0012 · Demo · P0 · S · Split sim from draw — entities expose state only; drawing moves to `src/render/surgery/*` so the sim imports nothing from `render/` (lint rule enforces) _(every kind’s look is registered by class in `src/render/surgery/registry.ts` (`drawer`, inherited hook by hook; `drawAs` for what `super.draw` was); 36 modules moved, with the boss HUD, boss ring and paint helpers; the fx and choir-state types now live in the sim. `no-restricted-imports` forbids render/ and art/ in src/surgery. Verified by `tests/sim/draw-trace.test.ts`: every campaign op, 29 more ops and a 34-kind zoo draw call-for-call as before)_
...
- [x] GAM-0195 · Demo · P0 · S · op2-1 Gravehound tuning — fang angles readable; claw lacerations total ≤ 5; steady rank B–A _(three claw rakes (tests/unit/content/clawRake.test.ts); the fang-nest angles fan 0.6–1.2 rad. Steady ranks S, not B–A: the calibrated thresholds (round(steady × 0.96), src/surgery/ranks.ts) supersede the B–A target across the campaign)_
- [x] GAM-0196 · Demo · P0 · S · op2-2 The Green Seam tuning — hexstone whisper drain never alone kills novice bot (min vitals ≥ 25)
- [x] GAM-0197 · Demo · P0 · S · op2-3 Brood-Mother's Kiss tuning — venom mote count and grub spawn cadence set so drain peaks ≤ 1.2/s steady _(one hatchling to a lanced sac, a clean lance leaves a nick (EGGSAC_WOUND), sacs hatch on a 22/32/42 s stagger; Ilvaren comes in at 80 so the chapter curve holds. tests/sim/brood-drain.test.ts: steady 5 s peak 1.17/s)_
...
- [x] GAM-0248 · Beta · P2 · M · Two-patient triage op (Ch4) — switch patient with Tab; each has separate vitals; fail if either dies
