# BOS — tasks that need a human

The boss framework (`MalisonBase`, declared tells, the phased Matins "Night Vigil" and Lauds
"Antiphon", the demo elites, boss HUD, codex and debriefs) is merged into the game branch; the
former WIP patch is gone. Balance is validated by `tests/sweep.test.ts` (20 seeds × 5 bot profiles),
`tests/hours.test.ts` and the characterisation suites. What remains needs playtesters or hardware:
