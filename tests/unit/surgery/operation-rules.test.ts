...
import { DT, defWith, drag, holdAt, isolate, makeOp, press, release, start, step, tap } from '../../helpers/sim';
...
describe('QAT-0020 victory bonus', () => {
  it('adds round(vitals)×20 + round(timeLeft)×10 exactly once', () => {
    ...
    expect(op.cues.filter((c) => c === 'bell')).toHaveLength(1);
  });
});
...
describe('QAT-0021 loss states', () => {
  it('vitals reaching 0 loses with "The patient has died."', () => {
    ...
    op.hurt(5);
    op.rate('cool', C);
    expect(op.cues.filter((c) => c === 'flatline')).toHaveLength(1);
...
describe('QAT-0023 Litany gating', () => {
  it('succeeds once per operation with exactly one cue', () => {
    ...
    expect(op.cues.filter((c) => c === 'litany')).toHaveLength(1);
  });

  it('fails when the op disables it', () => {
    const { op } = runningOp({ litany: false });
    expect(op.invokeLitany()).toBe(false);
    expect(op.cues).not.toContain('litany');
  });
...
describe('QAT-0024 tincture', () => {
  it('a 0.7 s hold on the body heals 25 and starts a 6 s cooldown', () => {
    ...
    expect(op.cues).toContain('inject');
  });
...
describe('QAT-0025 empty lancet press', () => {
  it('on bare body with work on the table: MISS, -3 vitals, cut cue', () => {
    ...
    expect(op.cues.filter((c) => c !== 'select')).toEqual(['miss', 'cut']);
  });
  ...
  it('outside the FIELD ellipse does nothing', () => {
    ...
    expect(op.cues.filter((c) => c !== 'select')).toEqual([]);
  });
...
describe('QAT-0028 damage feedback', () => {
  it('hurt() is ignored unless running', () => {
    ...
    expect(op.popups).toHaveLength(0);
  });

  it('caps shake at 12, decays it at 30/s, pops a red -N for hits ≥ 1 and expires popups after 1.1 s', () => {
    const { op } = runningOp();
    op.popups.length = 0;
    op.hurt(20, C);
    expect(op.shake).toBe(12);
    expect(op.popups).toEqual([expect.objectContaining({ text: '-20', color: '#c0392b' })]);
    op.hurt(0.5, C);
    expect(op.popups).toHaveLength(1);
    op.update(0.1);
