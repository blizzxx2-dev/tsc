...
describe('GAM-E fractures & bone-setting', () => {
  ...
  it('GAM-0095: the bot aligns and pins three procedural fractures in under 25 s', () => {
    for (const seed of [3, 4, 5]) {
      const op = playWithBot(testDef((o) => [new Fracture(at(0, 0), o, 0.3, 3 + (seed % 3))], { seed }), { profile: 'steady' }).op;
...
describe('GAM-E frost', () => {
  ...
  it('GAM-0101/0102: frozen flesh rejects lancet and thread; ice crystals come out only once thawed', () => {
    ...
    expect(lac.alive).toBe(true);
    expect(op.popups.some((p) => p.text.includes('Frozen'))).toBe(true);
...
describe('GAM-E tumours & growths', () => {
  ...
  it('GAM-0105: feeder vessels must be tied first or excision bleeds −15 each', () => {
    ...
    for (const ve of g2.vessels) {
      ...
      h.drag('thread', [{ x: mid.x - Math.cos(a) * 20, y: mid.y - Math.sin(a) * 20 }, { x: mid.x + Math.cos(a) * 20, y: mid.y + Math.sin(a) * 20 }], 250);
    }
...
describe('GAM-E ulcers, troll wounds, bites', () => {
  it('GAM-0110/0111: drain the bile, then salve outer ring first; inner-first perforates and the spill rots after 6 s', () => {
    ...
    expect(op.events.some((e) => e.kind === 'rated' && e.rating === 'bad' && e.label === 'Perforated')).toBe(true);
    ...
    for (let r = 0; r < 3; r++) for (let k = 0; k < 4 && u2.healed === r && u2.alive; k++) new Hand(ok).drag('salve', circlePath(u2.pos, u2.radius - r * 14 - 7, 40), 300);
...
describe('GAM-E acids, poisons, gangrene', () => {
  ...
  it('GAM-0123: gangrene creeps 2 px/s; caught below the line it is debrided and salved; past it, amputation', () => {
    ...
    const rows = [];
    for (let y = -cov.radius; y <= cov.radius; y += 14) rows.push({ x: cov.center.x - cov.radius - 6, y: cov.center.y + y }, { x: cov.center.x + cov.radius + 6, y: cov.center.y + y });
    h.drag('salve', rows, 900);
    expect(g.alive).toBe(false);
...
describe('GAM-E organ hazards', () => {
  ...
  it('GAM-0134: during arrhythmia the lancet near the heart only cuts between beats', () => {
    ...
    for (let x = -80; x <= 80; x += 0.5) {
      ...
      if (inc.progress === before && inc.state === 'mark') betweenBeats(op) ? open++ : blocked++;
    }
  ...
  it('GAM-0136: trepanation — three steady circles, too fast nicks the dura; then lift the disc', () => {
    ...
    expect(fast.events.some((e) => e.kind === 'rated' && e.label === 'Dura nicked')).toBe(true);
  });
  ...
  it('GAM-0246: an unclamped artery pools every second; pulling a shard beside it tears it', () => {
    ...
    h.release();
    wait(op, DT * 2);
    expect(op.events.some((x) => x.kind === 'rated' && x.label === 'Artery torn')).toBe(true);
...
describe('GAM-J bot extensions for Alpha mechanics', () => {
  const ops: [string, (op: Operation) => Entity[], Parameters<typeof testDef>[1]][] = [
    ...
    ['alchemy', (o) => [new AlchemicalAcid(at(-150, 0)), new CompoundPoison(at(100, 100), o, 4), new GasPocket(at(0, -60))], { tinctures: ['green', 'blue', 'amber'] }],
    ...
    ['infection', () => [new InfectionLine([at(-200, 100), at(-100, 50), at(0, 0), at(100, -50), at(200, -100)], 3), new SporeCrust(at(-150, -80)), new DungZone(at(150, 100))], {}],
    ['organs', (o) => [new CollapsedLung(at(-150, 0)), new Trepanation(at(150, 0)), new LarynxFold(at(0, 100)), new StomachLock(at(0, -80), o), new WaxClot(at(-250, 80)), new Artery(at(250, 80))], {}],
