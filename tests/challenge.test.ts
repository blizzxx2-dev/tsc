...
import { dailyBoardId, dailySeed, decodeChallenge, encodeChallenge, LOOM_MODULES, loomCode, loomOp, loomVerses, MUTATORS, parseLoomCode, withMutators, X_OPS, xOp, xOpDef, xOpOptions, xUnlocked } from '../src/content/challenge';
...
import { Anchor, running, wait } from './harness';
...
describe('GAM-L Symptom Loom', () => {
  ...
  it('GAM-0219: loom validator — every module combination is completable by the bot across seeds', () => {
    ...
    for (let s = 1; s <= seeds; s++) {
      ...
      if (op.status !== 'won') failures.push(`${loomCode(s * 7919)} (${loomVerses(s * 7919).map((m) => m.id).join('+')}): ${op.lostReason}`);
    }
  ...
  it('GAM-0222: custom challenges round-trip through a share code', () => {
    const c = { opId: 'op2-3', mods: { drain: 1.25, time: 0.8, hp: 1, tellSpeed: 1.1, addCadence: 1.5 }, mutators: ['candle', 'stroh'] as ('candle' | 'stroh')[] };
