...
describe('GAM-J per-operation tuning (demo)', () => {
  ...
  it('GAM-0198: op2-4 — sigils of 4–7 strokes; the 4 s regress never catches a novice', () => {
    ...
    const sigils = byId('op2-4').phases[1].spawn(op).filter((e): e is Sigil => e instanceof Sigil);
    ...
    for (const s of seeds(10)) {
      let regress = 0;
      const run = playWithBot(byId('op2-4'), { profile: 'novice', seed: 24000 + s, botSeed: s, collect: (e) => e.kind === 'hint' && e.key === 'sigil-regress' && regress++ }).op;
