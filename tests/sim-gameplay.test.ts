...
import { Anchor, at, Hand, running, testDef, wait, zig } from './harness-gameplay';
...
describe('GAM-B simulation hygiene', () => {
  ...
  it('GAM-0010: no tuning literals (other than 0/1) inside ailment update() bodies', () => {
    ...
    try {
      // Chapters III–V's ailment modules (hollownight, kilnrows, vennmark) predate the tuning table;
      // moving their literals into tuning.ts is follow-up work, so they are not scanned yet.
      const LATER = ['hollownight.ts', 'kilnrows.ts', 'vennmark.ts'];
      for (const f of readdirSync(join(ROOT, 'src/surgery/ailments'))) if (!LATER.includes(f)) files.push(join(ROOT, 'src/surgery/ailments', f));
    } catch {
    ...
    for (const f of files) {
      for (const body of updateBodies(readFileSync(f, 'utf8'))) {
        ...
        for (const lit of code.match(/(?<![\w.$])\d+(?:\.\d+)?(?![\w.])/g) ?? [])
          if (lit !== '0' && lit !== '1') offenders.push(`${f.split('/').pop()}: ${lit}`);
      }
    }
  ...
  it('GAM-0014: fixed-step sim — same step-indexed input gives the same result at 30/60/144 fps', () => {
    ...
    for (let i = 1; i < path.length; i++) {
      ...
      for (let k = 1; k <= n; k++)
        pts.push({ x: path[i - 1].x + ((path[i].x - path[i - 1].x) * k) / n, y: path[i - 1].y + ((path[i].y - path[i - 1].y) * k) / n });
    }
    const startStep = 300;
        ...
        op.advance(1 / fps, () => {
          if (step === 1400)
            snap = {
              ...
              lac: op.entities.filter((e) => e instanceof Laceration).length,
            };
  ...
  it('GAM-0015: recorded input replays to the exact same score and rank on all 10 demo ops', () => {
    for (const def of allOperations()) {
      ...
      expect([again.status, again.score, again.rank(), Math.round(again.vitals * 1000)], def.id).toEqual([
        orig.status,
        orig.score,
        orig.rank(),
        Math.round(orig.vitals * 1000),
      ]);
    }
  });
  ...
  it('GAM-0016: sim publishes typed events (rated, spawned, phaseStart, cue)', () => {
    ...
    const kinds = new Set(op.journal.map((e) => e.kind));
    expect(kinds.has('phaseStart')).toBe(true);
    expect(kinds.has('spawned')).toBe(true);
    const h = new Hand(op);
    h.drag('thread', zig(at(-30, 0), at(30, 0), 4), 300);
    const rated = op.journal.filter((e) => e.kind === 'rated');
    expect(rated.length).toBe(1);
    expect(op.journal.some((e) => e.kind === 'cue' && e.cue === 'stitch')).toBe(true);
  });
