...
describe('entity registry (CON-0002)', () => {
  const samples: [EntitySpec, new (...a: never[]) => unknown][] = [
    ...
    [{ e: 'incision', path: [[-50, 0], [50, 0]] }, Incision],
  ...
  it('validates parameter types and ranges', () => {
    ...
    expect(validateSpec({ e: 'embedded', at: [0, 0], kind: 'warpshard' }, ALL, 'x').join()).toMatch(/not one of/);
  ...
  it('seeded picks choose the same entities for the same seed', () => {
    ...
    const run = (seed: number) => spawnAll(list, start(() => [], { seed })).map((e) => e.pos.x);
...
describe('operation data validation (CON-0003)', () => {
  ...
  const base: OperationData = { id: 'bad', title: 'Bad', patient: 'P', diagnosis: 'D', organ: 'flesh', timeLimit: 60, tools: ['thread'], ranks: { S: 3, A: 2, B: 1 }, phases: [] };
  ...
  it('rejects positions off the operating field', () => {
    ...
    expect(validateOp({ ...base, tools: ['lancet', 'thread'], phases: [{ spawn: [{ e: 'incision', path: [[0, 0], [0, 400]] }] }] }).join()).toMatch(/leaves the operating field/);
  });
  ...
  it('rejects entities the op has no instrument for', () => {
    ...
    expect(validateOp({ ...base, tools: ['tongs'], phases: [{ spawn: [{ e: 'embedded', at: [0, 0], kind: 'shot', hidden: true }] }] }).join()).toMatch(/needs lens/);
  });
  ...
  it('rejects a closing phase with no incision, empty phases and inverted ranks', () => {
    ...
    expect(validateOp({ ...base, ranks: { S: 1, A: 2, B: 3 }, phases: [{ spawn: [{ e: 'laceration', at: [0, 0], angle: 0, len: 60 }] }] }).join()).toMatch(/S > A > B/);
  });
});
