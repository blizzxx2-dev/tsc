...
export const makeOp = (def: OperationDef): Operation => tapeOp(new Operation(def));

/** What the sim announced on its event bus, recorded for assertions (`length = 0` clears an array). */
export interface Tape {
  cues: string[];
  popups: SimEvents['popup'][];
  fx: SimEvents['fx'][];
}
const tapes = new WeakMap<Operation, Tape>();

/** Start recording an operation's bus events (makeOp does this for you). */
export function tapeOp(op: Operation): Operation {
  if (tapes.has(op)) return op;
  const t: Tape = { cues: [], popups: [], fx: [] };
  op.events.on('cue', (c) => t.cues.push(c));
  op.events.on('popup', (p) => t.popups.push(p));
  op.events.on('fx', (f) => t.fx.push(f));
  tapes.set(op, t);
  return op;
}

/** The recorded events of an operation (recording starts on first call if it hadn't already). */
export function tape(op: Operation): Tape {
  tapeOp(op);
  return tapes.get(op)!;
}
