...
export function drawSecondaryVitals(g: Gfx, op: Operation, x: number, y: number): void {
  ...
  let cx = x;
  if (s.bloodVolume) {
    g.text('BLOOD', cx, y + 4, { size: 10, color: hex(UI.brass), shadow: false });
...
export function drawLitanyPractice(g: Gfx, op: Operation, input: Input): boolean {
  ...
  g.text('The Litany of Stillness', VIEW_W / 2, 250, { size: 44, font: 'display', color: hex(UI.gilt), align: 'center' });
  g.text('Hold the right mouse button and draw a five-pointed star in one stroke.', VIEW_W / 2, 300, { size: 22, color: hex(UI.parch), align: 'center' });
  g.text(`${p.attempts} attempt${p.attempts === 1 ? '' : 's'} left`, VIEW_W / 2, 336, { size: 18, font: 'italic', color: hex(UI.parchLo), align: 'center' });
  return button(g, input, 'Let Ilse speak it for me', VIEW_W / 2, 420, 24);
}
...
export function drawDialogue(g: Gfx, op: Operation, input: Input): boolean {
  ...
  g.text('click to continue', r.x + r.w - 20, r.y + r.h - 8, { size: 12, font: 'italic', color: hex(UI.inkDark, 0.6), align: 'right', shadow: false });
  return input.pressed || input.keyPressed('Enter') || input.keyPressed('Space');
}
