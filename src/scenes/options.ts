...
export class OptionsScene implements Scene {
  ...
  private layout(game: Game): void {
    ...
    this.rows.forEach((row, i) => {
      ...
      const base = { id: `row${i}`, rect, label: t(row.label), clip: this.list.view, tip: undefined };
  ...
  render(g: Gfx, game: Game): void {
    ...
    if (row) {
      ...
      fitBlock(g, `options.${row.id}.note`, t(noteKey(row)), 230, 590, noteW, 1, { size: 18, font: 'italic', color: hex(palette().inkDim) });
