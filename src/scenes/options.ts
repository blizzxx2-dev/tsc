...
export class OptionsScene implements Scene {
  ...
  render(g: Gfx, game: Game): void {
    ...
    this.rows.forEach((row, i) => {
      ...
      g.text(t(row.label), r.x + 20, r.y + 34, { size: 24, color: hex(hover ? PALETTE.gold : PALETTE.ink) });
    ...
    if (note) g.text(t(note), VIEW_W / 2, 590, { size: 18, font: 'italic', color: hex(PALETTE.inkDim), align: 'center' });
    if (button(g, game.input, t('ui.common.back'), VIEW_W / 2, 645, 28)) this.back();
