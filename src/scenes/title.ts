  ...
  render(g: Gfx, game: Game): void {
    ...
    if (this.confirmNew) {
      ...
      if (IS_DEMO && flag('wishlistPrompts')) {
        ...
        if (platform.steam.ownsFullGame) g.text('The full game is in your library.', VIEW_W / 2, 630, { size: 20, font: 'italic', color: hex(UI.parch), align: 'center' });
        else if (button(g, game.input, 'Wishlist the Full Game', VIEW_W / 2, 630, 22)) platform.steam.openStore(EDITIONS.full.steamAppId);
      }
