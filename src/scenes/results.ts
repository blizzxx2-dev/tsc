...
export class ResultsScene implements Scene {
  ...
  render(g: Gfx, game: Game): void {
    ...
    const rows: [string, string][] = [
      ...
      ['Vitals kept', String(op.bonus.vitals)],
    ...
    if (this.won && this.t > 1.6) {
      ...
      if (k >= 1) {
        ...
        if (assisted() || op.resultFlags().length) g.text('(assisted)', sx, sy + 144, { size: 16, font: 'italic', color: faded, align: 'center', shadow: false });
      }
    } else if (!this.won) {
    ...
    if (this.t > 2) {
      ...
      if (this.summary?.tip) g.textBlock(`Sister Ilse: “${this.summary.tip}”`, r.x + 400, r.y + 440, 250, { size: 16, font: 'italic', color: hex('#6a0a10'), shadow: false }, 1.15);
    }

    if (this.t > 1) {
      if (!this.won && this.actions.retryCheckpoint && button(g, game.input, 'From the Malison', VIEW_W / 2 + 150, 618, 22)) this.actions.retryCheckpoint();
      if (!this.won && this.actions.retryNovice && button(g, game.input, 'Retry at Novice', VIEW_W / 2 - 150, 618, 22)) this.actions.retryNovice();
