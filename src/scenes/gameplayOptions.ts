...
export class GameplayOptionsScene implements Scene {
  ...
  render(g: Gfx, game: Game): void {
    ...
    g.text('Gameplay & Assists', VIEW_W / 2, 100, { size: 40, font: 'display', color: hex(PALETTE.ink), align: 'center' });
    g.text(`Fees: ${progress.fees} crowns`, 980, 100, { size: 16, font: 'italic', color: hex(PALETTE.gold), align: 'right' });
    ...
    if (button(g, game.input, 'Back', VIEW_W / 2, 672, 24)) this.back();
