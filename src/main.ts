...
import { isMenuPage, Transition, type TransitionOptions } from './ui/transition';
...
class Main implements Game {
  ...
  go(scene: Scene, opts?: TransitionOptions): void {
    ...
    else this.transition.request(() => this.goNow(scene), opts ?? (isMenuPage(this.scenes.top) && isMenuPage(scene) ? { transition: 'page' } : undefined));
  }
