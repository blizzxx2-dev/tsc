...
/** Patient voice type from the operation's patient line and race: 0 man, 1 woman, 2 elder, 3 dwarf. */
export function patientVoice(patient: string, race?: string): number {
  if (race === 'dwarf') return 3;
...
export class OperationAudio {
  ...
  private seenPopups = new WeakSet<Popup>();
  ...
  frame(f: OpFrame): void {
    ...
    const newPopups = op.popups.filter((p) => !this.seenPopups.has(p));
    for (const p of newPopups) this.seenPopups.add(p);
  ...
  private bosses(op: Operation, dt: number): void {
    ...
    if (lauds) {
      ...
      if (lauds.submerged && !st.submerged && !op.popups.some((p) => p.label === 'Wounded')) this.play('sfx.lauds.submerge', { pan: panOf(lauds.pos.x) });
      st.hymn = hymn;
