...
import { BloodPool, Embedded, Laceration, Rot, Sigil, Venom, Wadding } from './entities';
 ...
 * on the save) and step-gated guided operations. Matching is by class
 * (instanceof), never by name, so minified builds match too.
 */
...
type Cls = abstract new (...a: never[]) => Entity;
// Classes are referenced lazily (thunks): this module and the ailments import each other.
const is = (cls: () => Cls) => (e: Entity) => e instanceof cls();
const embed = (kind: string, pred: (e: Embedded) => boolean = () => true) => (e: Entity) => e instanceof Embedded && e.kind === kind && pred(e);

export const FIRST_HINTS: readonly FirstHint[] = [
  { id: 'barb', match: embed('arrow', (e) => e.barbed), text: 'A barbed head — two lancet nicks at the entry before you pull.' },
...
const first = (cls: () => Cls) => (op: Operation) => op.entities.find((e) => e.alive && !e.hidden && e instanceof cls())?.pos ?? null;
