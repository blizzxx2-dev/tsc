  ...
  draw(g: Gfx, op: Operation): void {
    ...
    switch (this.kind) {
      ...
      case 'hexstone': {
        ...
        if (warp) g.glow(x, y, 40, hex(this.calmed ? '#a04010' : '#ff6a20', 0.35));
        g.poly(
          [
            { x, y },
...
export class Burn extends Entity {
  ...
  draw(g: Gfx, op: Operation): void {
    ...
    if (this.source === 'hexfire') g.glow(x, y, this.radiusNow * 1.3, hex(this.smoulder >= 0 && this.smoulder < 0.8 && Math.sin(op.elapsed * 40) > 0 ? '#60ff80' : '#c060ff', 0.18 + 0.1 * Math.sin(op.elapsed * 7)));
